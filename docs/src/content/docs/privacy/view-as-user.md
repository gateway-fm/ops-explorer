...
When an operator views as a user, the explorer rewrites its calls to the Open Privacy Suite's
administrative impersonation path, naming the target user and organisation. The Open Privacy Suite
...
- **Scoped server-side.** The Open Privacy Suite enforces that the operator is allowed to impersonate
...
2. Set `SSO_JWKS_URL` to the Open Privacy Suite's JWKS endpoint (see
   [Authentication](../authentication/)).
3. Configure the operator's impersonation permissions in the Open Privacy Suite.

## The Open Privacy Suite side

The impersonation permissions, organisation scoping, and audit trail live in the Open Privacy Suite.
