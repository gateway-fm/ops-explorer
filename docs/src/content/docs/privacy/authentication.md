...
description: How sign-in works in privacy mode — the OAuth flow through the Open Privacy Suite, session cookies, JWT verification, and the SSO settings.
---

In privacy mode every user signs in through the Open Privacy Suite. The explorer acts as the OAuth
...
1. The user clicks sign in; the explorer redirects them to the Open Privacy Suite's OAuth endpoint
   (using `PRIVACY_PROXY_PUBLIC_URL`, the browser-facing address).
2. After authenticating, the Open Privacy Suite redirects back to the explorer's `SSO_REDIRECT_URI`
   with an authorization code, plus a `state` value that the explorer validates (CSRF
   protection).
3. The explorer exchanges the code for tokens **server-side**, calling the Open Privacy Suite's
...
the Open Privacy Suite *who* is asking, so the Open Privacy Suite can apply that user's redaction. It does this
...
the Open Privacy Suite, but a different place it happens.
:::
...
2. The explorer's API reads the JWT out of that cookie and carries it through the request
   context.
3. A transport layer wrapping every outbound call to the Open Privacy Suite attaches the token as an
   `Authorization: Bearer <jwt>` header.
4. The Open Privacy Suite reads that header, identifies the user, and redacts the response for them.
...
- **The same token is injected everywhere the explorer talks to the Open Privacy Suite**, including
...
**the Open Privacy Suite is always the authoritative verifier**: it re-validates the token on every
call before returning data, regardless of any local check.

## JWT verification

When `SSO_JWKS_URL` is set, the explorer fetches the Open Privacy Suite's signing keys and verifies
...
|----------|----------|------|
| `PRIVACY_PROXY_PUBLIC_URL` | Yes | Browser-facing Open Privacy Suite URL used for OAuth redirects, e.g. `https://proxy.yourdomain.com`. |
| `SSO_REDIRECT_URI` | Yes | OAuth callback on the explorer, e.g. `https://explorer.yourdomain.com/api/auth/callback`. |
| `SSO_CLIENT_ID` | No (default `explorer`) | OAuth client id. **Must match the client registered in the Open Privacy Suite.** |
| `SSO_CLIENT_SECRET` | Yes (OAuth) | Client secret for the token exchange. **Source from a secrets manager, never inline.** |
| `SSO_JWKS_URL` | Recommended | The Open Privacy Suite JWKS endpoint. Enables in-process JWT verification and [View as user](../view-as-user/). |
...
`SSO_CLIENT_ID` must match the client identifier registered on the Open Privacy Suite side. A
mismatch fails the token exchange and blocks all sign-in.
:::

For the Open Privacy Suite's side of the OAuth/SSO configuration, see its
