...
description: How sign-in works in privacy mode — the OAuth flow through privacy-proxy, session cookies, JWT verification, and the SSO settings.
---

In privacy mode every user signs in through privacy-proxy. The explorer acts as the OAuth
...
1. The user clicks sign in; the explorer redirects them to privacy-proxy's OAuth endpoint
   (using `PRIVACY_PROXY_PUBLIC_URL`, the browser-facing address).
2. After authenticating, privacy-proxy redirects back to the explorer's `SSO_REDIRECT_URI`
   with an authorization code, plus a `state` value that the explorer validates (CSRF
   protection).
3. The explorer exchanges the code for tokens **server-side**, calling privacy-proxy's
...
2. The explorer's API reads the JWT out of that cookie and carries it through the request
   context.
3. A transport layer wrapping every outbound call to privacy-proxy attaches the token as an
   `Authorization: Bearer <jwt>` header.
4. Privacy-proxy reads that header, identifies the user, and redacts the response for them.
...
- **The same token is injected everywhere the explorer talks to privacy-proxy**, including
...
**privacy-proxy is always the authoritative verifier**: it re-validates the token on every
call before returning data, regardless of any local check.

## JWT verification

When `SSO_JWKS_URL` is set, the explorer fetches privacy-proxy's signing keys and verifies
...
|----------|----------|------|
| `PRIVACY_PROXY_PUBLIC_URL` | Yes | Browser-facing privacy-proxy URL used for OAuth redirects, e.g. `https://proxy.yourdomain.com`. |
| `SSO_REDIRECT_URI` | Yes | OAuth callback on the explorer, e.g. `https://explorer.yourdomain.com/api/auth/callback`. |
| `SSO_CLIENT_ID` | No (default `explorer`) | OAuth client id. **Must match the client registered in privacy-proxy.** |
| `SSO_CLIENT_SECRET` | Yes (OAuth) | Client secret for the token exchange. **Source from a secrets manager, never inline.** |
| `SSO_JWKS_URL` | Recommended | privacy-proxy JWKS endpoint. Enables in-process JWT verification and [View as user](../view-as-user/). |
...
`SSO_CLIENT_ID` must match the client identifier registered on the privacy-proxy side. A
mismatch fails the token exchange and blocks all sign-in.
:::

For privacy-proxy's side of the OAuth/SSO configuration, see its
[authentication docs](https://gateway-fm.github.io/privacy-proxy/docs/authentication/).
