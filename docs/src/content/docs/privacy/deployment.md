...
description: The fail-closed configuration, security hardening, and rollout steps for running the block explorer behind the Open Privacy Suite proxy.
---
...
# The external Open Privacy Suite proxy network must exist before starting
...
- **The Open Privacy Suite proxy is a hard dependency.** If it is unreachable, authenticated endpoints
...
- **Client id alignment.** `SSO_CLIENT_ID` (default `explorer`) must match the client
  registered in the Open Privacy Suite proxy, or sign-in fails. See [Authentication](../authentication/).
- **Database role.** The explorer's own Postgres is used for contract-verification metadata
  only in mixed deployments; all chain data comes from the Open Privacy Suite proxy.

## Open Privacy Suite documentation

The redaction, identity, and access-control rules live in the Open Privacy Suite proxy:
