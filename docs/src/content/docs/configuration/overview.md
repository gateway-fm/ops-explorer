...
| `PRIVACY_PROXY_URL` | One of these | Privacy mode — internal URL of the Open Privacy Suite proxy backend. |
...
| `VITE_RPC_URL` | Standalone | The node's **browser-facing public JSON-RPC URL** a wallet connects to directly in standalone mode. A node concern, **not** the Open Privacy Suite proxy. Falls back to `http://localhost:8545`. |
...
| `SSO_CLIENT_ID` | `explorer` | OAuth client id (privacy mode — must match the Open Privacy Suite proxy). |
