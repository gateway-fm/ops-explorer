...
-- Transactions (enhanced with nonce, tx_type, error handling)
...
CREATE TABLE IF NOT EXISTS tokens (
    ...
    token_type TEXT NOT NULL DEFAULT 'ERC20', -- ERC20, ERC721, NATIVE
...
-- Token Transfers (enhanced with transfer type, timestamp)
CREATE TABLE IF NOT EXISTS token_transfers (
    ...
    block_number BIGINT NOT NULL,
    timestamp BIGINT,
    transfer_type TEXT DEFAULT 'transfer', -- transfer, mint, burn, deposit, withdrawal
    token_type TEXT DEFAULT 'ERC20', -- ERC20, ERC721
    token_id NUMERIC(78, 0), -- For ERC721 NFTs
...
-- Balances (historical balance tracking per block)
CREATE TABLE IF NOT EXISTS balances (
    address TEXT NOT NULL,
    token_address TEXT NOT NULL, -- Use 'ETH' or '0x0' for native token
    block_number BIGINT NOT NULL,
...
-- Counters (pre-computed aggregation counts)
CREATE TABLE IF NOT EXISTS counters (
    address TEXT NOT NULL,
    counter_type TEXT NOT NULL, -- transactions, transfers, token_transfers, internal_txs
...
-- Contracts (enhanced with verification status)
...
-- Block sync status (for tracking indexer progress and reorgs)
